...
describe('csvParser', () => {
  describe('parseCSV', () => {
    it('should parse valid CSV data correctly', async () => {
      const csvContent = `id,name,startDate,endDate,progress,status,assignee,profile_type,remaining_estimate_hours,dependency
...
task-1,Test Task,2024-01-15,2024-02-20,100,completed,Jane Smith,frontend,0,`
    ...
    it('should calculate project start and end dates correctly', async () => {
      const csvContent = `id,name,startDate,endDate,progress,status,assignee,profile_type,remaining_estimate_hours,dependency
...
task-3,Task 3,2024-02-15,2024-03-01,0,not-started,Bob,qa,100,`
    ...
    it('should calculate total planned work from remaining hours', async () => {
      const csvContent = `id,name,startDate,endDate,progress,status,assignee,profile_type,remaining_estimate_hours,dependency
task-1,Task 1,2024-01-01,2024-01-10,100,completed,John,backend,0,
task-2,Task 2,2024-01-15,2024-02-20,50,in-progress,Jane,frontend,50,
task-3,Task 3,2024-02-15,2024-03-01,0,not-started,Bob,qa,100,`
...
task-4,Task 4,2024-01-15,2024-01-25,0,not-started,Alice,devops,80,task-3`
    ...
    it('should handle default values for remaining_estimate_hours', async () => {
      const csvContent = `id,name,startDate,endDate,progress,status,assignee,profile_type,remaining_estimate_hours,dependency
task-1,Task 1,2024-01-01,2024-01-10,100,completed,John,backend,,`

      const result = await parseCSV(csvContent)

      expect(result.tasks[0].remaining_estimate_hours).toBe(0)
    })

    it('should set name to "Untitled Task" if missing', async () => {
      const csvContent = `id,name,startDate,endDate,progress,status,assignee,profile_type,remaining_estimate_hours,dependency
task-1,,2024-01-01,2024-01-10,50,in-progress,John,backend,40,`

      const result = await parseCSV(csvContent)

      expect(result.tasks[0].name).toBe('Untitled Task')
    })
  })
})
